...
@section('content')
    <div class="min-h-screen py-8 px-4 md:px-8">
        ...
        <div class="text-center mb-10">
            <h1 class="text-4xl font-bold text-gray-800 mb-3">Rekap Mahasiswa PKL Kaprodi</h1>
        ...
        <div class="bg-white rounded-xl shadow-lg p-8 mb-6">
            <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div class="flex items-end space-x-2 mt-4 md:mt-0">
                    ...
                    <a href="{{ route('kaprodi.rekapMahasiswaPKL.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">Reset</a>
                </div>
            </form>
        </div>
                ...
                <table class="w-full table-fixed">
                    <thead class="bg-gray-50 border-b">
                        <tr>
                            ...
                            <th class="px-2 py-4 text-center text-xs font-bold text-gray-900 uppercase tracking-wider whitespace-nowrap">ALAMAT PERUSAHAAN</th>
                            <th class="px-2 py-4 text-center text-xs font-bold text-gray-900 uppercase tracking-wider whitespace-nowrap">ACTION</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200">
                        @forelse($pengajuans as $index => $pengajuan)
                            <tr class="bg-white hover:bg-gray-50">
                                ...
                                <td class="px-2 py-5 text-sm font-semibold text-gray-500 text-center max-w-[120px] truncate whitespace-normal" title="{{ $pengajuan->perusahaan->alamat ?? '-' }}">{{ $pengajuan->perusahaan->alamat ?? '-' }}</td>
                                <td class="px-2 py-5 text-sm font-semibold text-center whitespace-nowrap">
                                    <a href="{{ route('kaprodi.rekapMahasiswaPKL.show', $pengajuan->id) }}" class="inline-flex items-center justify-center w-8 h-8 text-blue-600 hover:text-blue-800 transition-colors duration-200" title="Detail">
