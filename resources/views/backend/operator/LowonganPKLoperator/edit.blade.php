...
@section('content')
    <div class="min-h-screen py-8 px-4 md:px-8">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
            <form action="{{ route('operator.lowonganPKL.update', $lowonganPKL->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="perusahaan_id" id="perusahaan_id" required
                            ...
                            @foreach($perusahaans as $perusahaan)
                                <option value="{{ $perusahaan->id }}" {{ (old('perusahaan_id', $lowonganPKL->perusahaan_id) == $perusahaan->id) ? 'selected' : '' }}>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="deskripsi" id="deskripsi" rows="4" required
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                            placeholder="Masukkan deskripsi lowongan">{{ old('deskripsi', $lowonganPKL->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="md:col-span-2">
                        <label for="syarat" class="block text-sm font-medium text-gray-700 mb-2">Syarat</label>
                        <textarea name="syarat" id="syarat" rows="4" required
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                            placeholder="Masukkan syarat lowongan">{{ old('syarat', $lowonganPKL->syarat) }}</textarea>
                        @error('syarat')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
