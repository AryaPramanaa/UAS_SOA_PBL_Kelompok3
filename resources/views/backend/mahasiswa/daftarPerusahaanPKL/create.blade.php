...
@section('title', 'Tambah Perusahaan PKL')
...
@section('content')
    <div class="min-h-screen py-8 px-4 md:px-8">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
            <form action="{{ route('mahasiswa.daftarPerusahaanPKL.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="kontak" id="kontak" 
                            ...
                            value="{{ old('kontak') }}" required placeholder="Masukkan kontak perusahaan">
                        @error('kontak')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="bidang_usaha" id="bidang_usaha" 
                            ...
                            value="{{ old('bidang_usaha') }}" required placeholder="Masukkan bidang usaha">
                        @error('bidang_usaha')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="alamat" id="alamat" rows="3" 
                            ...
                            required placeholder="Masukkan alamat lengkap perusahaan">{{ old('alamat') }}</textarea>
                        @error('alamat')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
