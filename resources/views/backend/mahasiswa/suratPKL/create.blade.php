...
@section('content')
    <div class="min-h-screen py-8 px-4 md:px-8">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
            <form action="{{ route('mahasiswa.suratPKL.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Nama Mahasiswa</label>
                        <input type="text" value="{{ Auth::user()->mahasiswa->nama ?? '-' }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-100" readonly>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">NIM</label>
                        <input type="text" value="{{ Auth::user()->mahasiswa->nim ?? '-' }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-100" readonly>
                    </div>
